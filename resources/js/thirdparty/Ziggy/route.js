...
class LaravelRouter extends String {
    constructor(name, params, absolute, customZiggy=null) {
        console.log('LaravelRouter:constructor:', name, params);
        super();
    ...
    current(name = null) {
        ...
        let currentRoute = routeNames.filter(name => {
            return new LaravelRouter(name, undefined, undefined, this.ziggy).matchUrl();
        })[0];
...
export default function route(name, params, absolute, customZiggy) {
    return new LaravelRouter(name, params, absolute, customZiggy);
};